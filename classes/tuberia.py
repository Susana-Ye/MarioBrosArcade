class Tuberia:
    def __init__(self, w: int, h: int):
        """
        @Constantes
        TUBO_X: tupla que almacena la información correspondiente al sprite
        de cada tubería (x, y, banco, u, v, w, h, tipo de tubo)

        @atributo sprite: Lista que guarda la información de las 4 tuberías
        """
        TUBO_0 = (0, h // 4 - 36, 0, 68, 80, 64, 36, 'spawn_enemigo_izq')
        TUBO_1 = (w - 44, h // 4 - 36, 0, 64, 32, 64, 36,
                  'spawn_enemigo_der')
        TUBO_2 = (0, h - 40, 0, 0, 88, 32, 24, 'entrada_izq')
        TUBO_3 = (w - 32, h - 40, 0, 0, 64, 32, 24,
                 'entrada_der')
    ...
    def es_tubo_entrada(self, x_ene: int, y_ene: int, w_ene: int, dir_ene: bool):
        ...
        if dir_ene and ((self.sprite[3][0] <= x_ene + w_ene <=
                               self.sprite[3][0] + self.sprite[3][5]) and
                               (self.sprite[3][1] <= y_ene <=
                               self.sprite[3][1] + self.sprite[3][6])):
            es_tubo_entrada = True
            x_tubo = self.sprite[3][0]
            y_tubo = self.sprite[3][1]
            tubo = 'right'
        # Si el enemigo está moviéndose hacia la izquierda
        elif not dir_ene and ((self.sprite[2][0] <= x_ene <=
                               self.sprite[2][0] + self.sprite[2][5]) and
                               (self.sprite[2][1] <= y_ene <=
                               self.sprite[2][1] + self.sprite[2][6])):
            es_tubo_entrada = True
